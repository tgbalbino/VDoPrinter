...
type
  ...
  TVDOBaseFont = class(TPersistent)

  end;

  TVDOBasePrinter = class(TComponent)
  private
    //FFromPage, FToPage, FCopies : Integer;
...
begin
  ...
  //PrintDlg.PrintRange := prPageNums;

{  PrintDlg.FromPage   := 0;
  PrintDlg.ToPage     := FToPage;
  PrintDlg.Collate    := FCollate;
  PrintDlg.Options    := FOptions;
  PrintDlg.PrintToFile:= FPrintToFile;
  PrintDlg.PrintRange := FPrintRange;
  PrintDlg.MinPage := FMinPage;
  PrintDlg.MaxPage := FMaxPage;
  PrintDlg.Copies  := FCopies;
}
