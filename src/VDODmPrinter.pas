...
type
  ...
  private
    ...
    procedure WriteOnPrinter(AText: AnsiString);
  ...
  public
    ...
    procedure Replicate(ALine, AColumn, ACount: Integer;
      AChar: Char = '-'; ALineFeed: Boolean = True); overload;
    procedure WriteStr(AText: AnsiString; APreviewText: string = '');
  published
...
procedure TVDODmPrinter.WriteOnPrinter(AText: AnsiString);
var
  Len, Count: DWord;
begin
...
begin
  ...
  if OpenPrinter(PWideChar(PrinterName), FPrinterHandle, @PrnDefs) then
  begin
    with DocInfo do
    begin
      pDocName := PWideChar(Title);
      pOutputFile := nil;
      pDatatype := 'RAW';
    end;
...
procedure TVDODmPrinter.Print(AColumn: Integer; AText: string; ALineFeed: Boolean = False);
var
  Pad: string;
  Cols: Integer;
begin
...
procedure TVDODmPrinter.WriteStr(AText: AnsiString; APreviewText: string = '');
begin
  if ShowPreview then
  begin
    ParsePreviewFont;
