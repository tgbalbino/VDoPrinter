...
uses
  ...
  VDODmPrinter, VDOCaPrinter, Richedit, System.ImageList;
...
type
  ...
  public
    ...
    property SaveStream: TStream read FSaveStream write FSaveStream;
  end;
  { **** }
  ...
  public
    ...
    procedure Write(AText: AnsiString); overload;
    procedure Write(AText: AnsiString; AColumn: Integer; AFont: TVDOPreviewFont); overload;
    procedure Write(AText, ADmText: AnsiString; AFont: TVDOPreviewFont); overload;
  end;
...
begin
  try
    ...
    begin
      ...
      //Str.LoadFromStream(Page.Stream);
      //Str.SaveToFile('D:\teste.txt');
      //Page.Stream.Position := 0;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        redPreviewPage.SelStart := TextBlock.Start;
        redPreviewPage.SelLength := TextBlock.Len;
    ...
    begin
      ...
      begin
        ...
        try
          ...
          begin
            Page := PageList[I];
            Page.SaveStream.Position := 0;
            FS.CopyFrom(Page.SaveStream, Page.SaveStream.Size);
          end;
        finally
          FS.Free;
          Screen.Cursor := crDefault;
...
procedure TfrmVDOPreview.Write(AText: AnsiString);
var
  AuxText : AnsiString;
begin
  if AText = #10 then
    begin
      AuxText := #13 + AText;
      Page.SaveStream.Write(PWideChar(AuxText)^, Length(AuxText))
    end
  else
    Page.SaveStream.Write(PWideChar(AText)^, Length(AText));

  Page.Stream.Write(PWideChar(AText)^, Length(AText));
end;

procedure TfrmVDOPreview.Write(AText: AnsiString; AColumn: Integer; AFont: TVDOPreviewFont);
begin
  TextBlock := TVDOTextBlock.Create;
  with TextBlock do
  begin
...
procedure TfrmVDOPreview.Write(AText, ADmText: AnsiString; AFont: TVDOPreviewFont);
begin
  TextBlock := TVDOTextBlock.Create;
  with TextBlock do
  begin
  ...
  Page.Stream.Write(PWideChar(AText)^, Length(AText));
  Page.SaveStream.Write(PWideChar(AText)^, Length(AText));
  Page.DmStream.Write(PWideChar(ADmText)^, Length(ADmText));
end;
...
begin
  ...
  begin
    if Application.MessageBox(PWideChar(SPreviewClose), PWideChar(SConfirm), mb_IconQuestion + mb_YesNo + mb_DefButton1) = idYes then
      Close;
  end;
